...
class CartHeaderItem extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        Reactotron.log('after push', nextProps);
        setTimeout(() => this.setState({ checked: nextProps.checkAll && this.props.hideCheck ? true : false }), 0)

        if (nextProps.checkAll){
            this.props.pushSelectedChecks(this.props.item.cart_id, this.props.item.price);
        } else{
            this.props.pullSelectedChecks(this.props.item.cart_id, this.props.item.price);
        }
    }

    render() {
        Reactotron.log('is checked', this.props.checkAll, this.state.checked )
        return (
            <View style={[ styles.acorrHeader , {borderBottomRightRadius: this.props.expanded ? 0 : 20 }]}>
                <View style={styles.directionRow}>
                    ...
                    <View>
                        ...
                        <Text style={[styles.type , styles.aSFS ,{color:COLORS.labelBackground}]}>{this.props.item.price} { i18n.t('RS') }</Text>
                    </View>
                </View>
